...
### v1.6.0
- 加強「創新」部分：新增「現行做法 vs 本計劃」對比表建議
- 新增「不重複／不重疊資助」聲明規則，避免與現有政府計劃或學校已有計劃重複
- 新增「學校現有資源投入／非政府配套資源」提示，區分三種資源類型，不強制視為等額資助
- 搜索策略加入施政報告／財政預算案作為補充政策來源
- 加入「跨主題計劃」判斷，但只在實質符合時使用，不硬套主題
- 驗證清單新增 5 項自檢項
...
| **手動刪除重裝** | 刪除本地 skill 資料夾，然後重新安裝 | 任何用戶 |

> ⚠️ **注意**：Skill 不支持 AI 自己改自己的文件。更新必須通過平台的 skill 安裝機制完成。
