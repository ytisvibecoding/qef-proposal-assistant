...
CodeBuddy/WorkBuddy 的 skill 不支持 AI 自己改自己的文件，更新必須重新安裝：
...
AI 會自動從 GitHub 下載最新版並覆蓋本地文件。**這就是更新方式。**

> 💡 每次啟動 skill 時，AI 會自動檢查版本，發現新版會提示你重新安裝。
...
| 方式 | 命令/操作 | 適合誰 |
|------|----------|--------|
| **重新安裝（推薦）** | 對話中說「安裝 skill：https://github.com/ytisvibecoding/qef-proposal-assistant」 | 所有用戶，最簡單 |
| **AI 自動提醒** | 每次啟動自動檢查，有新版會提示重新安裝 | 所有用戶，零操作 |
| **Bash 腳本** | `curl -sL .../install.sh \| bash` | 有終端的用戶 |
| **Bash 強制覆蓋** | `curl -sL .../install.sh \| bash -s -- --force` | 版本不對時使用 |
| **手動刪除重裝** | `rm -rf ~/.codebuddy/skills/qef-proposal-assistant` 然後重新安裝 | 任何用戶 |

> ⚠️ **注意**：CodeBuddy/WorkBuddy 的 skill 不支持 AI 自己改自己的文件。寫在 SKILL.md 裡的「WebFetch→Write 覆蓋」只是文字說明，不會被 AI 執行。更新必須通過平台的 skill 安裝機制完成。
